...
* ctx发送的用户消息不会是不会超时的
 */
func (ctx *actorContext) sendUserMessage(pid PID, message MessageEnvelope) (err error) {
	if pid == nil {
		err = fmt.Errorf("##CTX sendUserMessage: pid is nil")
	} else {
		if chain := ctx.props.senderMiddlewareChain; nil != chain {
			err = chain(context.Background(), ctx, pid, message)
		} else {
...
func (ctx *actorContext) Send(pid PID, message interface{}) (err error) {
	return ctx.sendUserMessage(pid, WrapEnvelope(message))
}
...
func (ctx *actorContext) Respond(message interface{}) error {
	return ctx.Send(ctx.Sender(), message)
}

func (ctx *actorContext) Bubble(message interface{}) error {
	return ctx.Send(ctx.Parent(), message)
}
...
func (ctx *actorContext) Forward(pid PID) (err error) {
	...
	default:
		err = ctx.Send(pid, ctx.messageOrEnvelope)
	}
	return
}

func (ctx *actorContext) Request(pid PID, message interface{}) error {
	return ctx.RequestWithCustomSender(pid, message, ctx.Self())
}
